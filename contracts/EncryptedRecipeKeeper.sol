...
contract EncryptedRecipeKeeper is SepoliaConfig {
    ...
    function submitRecipe(
        ...
        for (uint256 i = 0; i < _recipeIngredients[recipeId].length; i++) {
            if (_recipeIngredients[recipeId][i].isEncrypted) {
                FHE.allow(_recipeIngredients[recipeId][i].encryptedAmount, msg.sender);
            }
        }
